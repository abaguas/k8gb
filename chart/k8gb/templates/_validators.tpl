...
{{- if and (or (not .Values.k8gb.dnsZones) (eq (len .Values.k8gb.dnsZones) 0)) .Values.k8gb.dnsZone .Values.k8gb.edgeDNSZone }}
  {{- $k8gbZones = append $k8gZones .Values.k8gb.edgeDNSZone -}}
{{- end }}

{{- $extdnsZones := .Values.extdns.domainFilters -}}

{{- if ne (len $k8gbZones) (len $extdnsZones) -}}
  {{- fail (printf "Validation failed: Number of zones in k8gb.edgeDNSZone/k8gb.dnsZones (%d) does not match number of domains in extdns.domainFilters (%d)" (len $k8gbZones) (len $extdnsFilters)) -}}
{{- end -}}

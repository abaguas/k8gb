...
func (rr *ReferenceResolver) GetGslbExposedIPs(edgeDNSServers utils.DNSList) ([]net.IP, error) {
	gslbIngressIPs := []net.IP{}

	for _, lb := range rr.lbService.Status.LoadBalancer.Ingress {
		ip := net.ParseIP(lb.IP)
		if len(ip) > 0 {
			gslbIngressIPs = append(gslbIngressIPs, ip)
		}
		if len(lb.Hostname) > 0 {
			IPs, err := utils.Dig(lb.Hostname, edgeDNSServers...)
			if err != nil {
